// Copyright (c) 2013-2014 The ohmcsuite developers
...
// oldohmcdHomeDir returns the OS specific home directory ohmcd used prior to
// version 0.3.3.  This has since been replaced with ohmcutil.AppDataDir, but
// this function is still provided for the automatic upgrade path.
func oldohmcdHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "ohmcd")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".ohmcd")
	}
...
// location prior to ohmcd version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to ohmcd
...
func upgradeDBPaths() error {
	...
	oldDbRoot := filepath.Join(oldohmcdHomeDir(), "db")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "ohmcd.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "ohmcd_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "ohmcd_regtest.db"), "regtest")

	// Remove the old db directory.
	return os.RemoveAll(oldDbRoot)
}

// upgradeDataPaths moves the application data from its location prior to ohmcd
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		ohmcdLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old ohmcd.conf into new location if needed.
		...
		if ohpEmpty {
			...
			ohmcdLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to ohmcd as new versions require it.
