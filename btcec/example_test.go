...
func Example_signMessage() {
	...
	privKey, pubKey := ohmcec.PrivKeyFromBytes(ohmcec.S256(), pkBytes)
...
func Example_verifySignature() {
	...
	pubKey, err := ohmcec.ParsePubKey(pubKeyBytes, ohmcec.S256())
	if err != nil {
		fmt.Println(err)
		return
	}
...
func Example_encryptMessage() {
	...
	message := "test message"
	ciphertext, err := ohmcec.Encrypt(pubKey, []byte(message))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	privKey, _ := ohmcec.PrivKeyFromBytes(ohmcec.S256(), pkBytes)

	// Try decrypting and verify if it's the same message.
	plaintext, err := ohmcec.Decrypt(privKey, ciphertext)
	if err != nil {
		fmt.Println(err)
		return
	}
