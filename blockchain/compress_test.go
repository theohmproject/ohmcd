...
func TestAmountCompression(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "0 ohmc (sometimes used in nulldata)",
			uncompressed: 0,
			compressed:   0,
		},
		{
		...
		{
			name:         "0.00001 ohmc (typical transaction fee)",
			uncompressed: 1000,
			compressed:   4,
		},
		{
			name:         "0.0001 ohmc (typical transaction fee)",
			uncompressed: 10000,
			compressed:   5,
		},
		{
			name:         "0.12345678 ohmc",
		...
		{
			name:         "21000000 ohmc (max minted coins)",
...
func TestCompressedTxOut(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "pay-to-pubkey uncompressed 1 ohmc",
			amount:     100000000,
