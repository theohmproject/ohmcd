...
func addrToKey(addr ohmcutil.Address) ([addrKeySize]byte, error) {
	...
	case *ohmcutil.AddressWitnessScriptHash:
		var result [addrKeySize]byte
		...
		// push to 20-bytes.
		copy(result[1:], ohmcutil.Hash160(addr.ScriptAddress()))
		return result, nil

	case *ohmcutil.AddressWitnessPubKeyHash:
		var result [addrKeySize]byte
...
type AddrIndex struct {
	...
	txnsByAddr      map[[addrKeySize]byte]map[chainhash.Hash]*ohmcutil.Tx
...
// the passed map.
func (idx *AddrIndex) indexBlock(data writeIndexData, block *ohmcutil.Block,
...
//
// This is part of the Indexer interface.
func (idx *AddrIndex) ConnectBlock(dbTx database.Tx, block *ohmcutil.Block,
...
//
// This is part of the Indexer interface.
func (idx *AddrIndex) DisconnectBlock(dbTx database.Tx, block *ohmcutil.Block,
	stxos []blockchain.SpentTxOut) error {
...
func (idx *AddrIndex) TxRegionsForAddress(dbTx database.Tx, addr ohmcutil.Address, numToSkip, numRequested uint32, reverse bool) ([]database.BlockRegion, uint32, error) {
	addrKey, err := addrToKey(addr)
	if err != nil {
		return nil, 0, err
	}
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) indexUnconfirmedAddresses(pkScript []byte, tx *ohmcutil.Tx) {
	...
	for _, addr := range addresses {
		...
		if addrIndexEntry == nil {
			addrIndexEntry = make(map[chainhash.Hash]*ohmcutil.Tx)
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) AddUnconfirmedTx(tx *ohmcutil.Tx, utxoView *blockchain.UtxoViewpoint) {
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) UnconfirmedTxnsForAddress(addr ohmcutil.Address) []*ohmcutil.Tx {
	// Ignore unsupported address types.
	addrKey, err := addrToKey(addr)
	if err != nil {
		return nil
	...
	if txns, exists := idx.txnsByAddr[addrKey]; exists {
		addressTxns := make([]*ohmcutil.Tx, 0, len(txns))
		for _, tx := range txns {
...
func NewAddrIndex(db database.DB, chainParams *chaincfg.Params) *AddrIndex {
	return &AddrIndex{
		...
		txnsByAddr:  make(map[[addrKeySize]byte]map[chainhash.Hash]*ohmcutil.Tx),
