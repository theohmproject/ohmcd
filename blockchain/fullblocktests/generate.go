...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: ohmcutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func makeTestGenerator(params *chaincfg.Params) (testGenerator, error) {
	privKey, _ := ohmcec.PrivKeyFromBytes(ohmcec.S256(), []byte{0x01})
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := ohmcutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*ohmcutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, ohmcutil.NewTx(tx))
	}
...
func additionalCoinbase(amount ohmcutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee ohmcutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee ohmcutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee ohmcutil.Amount) *wire.MsgTx {
...
func encodeNonCanonicalBlock(b *wire.MsgBlock) []byte {
	...
	b.Header.ohmcEncode(&buf, 0, wire.BaseEncoding)
	buf.Write(nonCanonicalVarInt(uint32(len(b.Transactions))))
	for _, tx := range b.Transactions {
		tx.ohmcEncode(&buf, 0, wire.BaseEncoding)
	}
	return buf.Bytes()
}
