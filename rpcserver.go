...
var (
	...
	// gohmcoinbaseAux describes additional data that miners should include
	...
	// data.
	gohmcoinbaseAux = &ohmcjson.GetBlockTemplateResultAux{
		Flags: hex.EncodeToString(builderScript(txscript.
			NewScriptBuilder().
			AddData([]byte(mining.CoinbaseFlags)))),
	}

	// gohmcapabilities describes additional capabilities returned with a
	// block template generated by the getblocktemplate RPC.    It is
	// declared here to avoid the overhead of creating the slice on every
	// invocation for constant data.
	gohmcapabilities = []string{"proposal"}
)
...
var (
	...
	ErrRPCUnimplemented = &ohmcjson.RPCError{
		Code:    ohmcjson.ErrRPCUnimplemented,
		Message: "Command unimplemented",
	}

	// ErrRPCNoWallet is an error returned to RPC clients when the provided
	// command is recognized as a wallet command.
	ErrRPCNoWallet = &ohmcjson.RPCError{
		Code:    ohmcjson.ErrRPCNoWallet,
...
// list of commands that we recognize, but for which ohmcd has no support because
// it lacks support for wallet functionality. For these commands the user
// should ask a connected instance of ohmcwallet.
...
// not needed.
func internalRPCError(errStr, context string) *ohmcjson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return ohmcjson.NewRPCError(ohmcjson.ErrRPCInternal.Code, errStr)
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *ohmcjson.RPCError {
	return ohmcjson.NewRPCError(ohmcjson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *ohmcjson.RPCError {
	return ohmcjson.NewRPCError(ohmcjson.ErrRPCNoTxInfo,
...
// These commands will be implemented in ohmcwallet.
...
func messageToHex(msg wire.Message) (string, error) {
	...
	if err := msg.ohmcEncode(&buf, maxProtocolVersion, wire.WitnessEncoding); err != nil {
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ohmcjson.CreateRawTransactionCmd)
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > ohmcutil.MaxSatoshi {
			return nil, &ohmcjson.RPCError{
				Code:    ohmcjson.ErrRPCType,
				Message: "Invalid amount",
			}
		}

		// Decode the provided address.
		addr, err := ohmcutil.DecodeAddress(encodedAddr, params)
		if err != nil {
			return nil, &ohmcjson.RPCError{
		...
		satoshi, err := ohmcutil.NewAmount(amount)
		if err != nil {
...
// transaction.
func createVinList(mtx *wire.MsgTx) []ohmcjson.Vin {
	// Coinbase transactions only have a single txin by definition.
	vinList := make([]ohmcjson.Vin, len(mtx.TxIn))
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.ScriptSig = &ohmcjson.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []ohmcjson.Vout {
	voutList := make([]ohmcjson.Vout, 0, len(mtx.TxOut))
	for i, v := range mtx.TxOut {
		...
		var vout ohmcjson.Vout
		vout.N = uint32(i)
		vout.Value = ohmcutil.Amount(v.Value).Toohmc()
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	blkHeight int32, chainHeight int32) (*ohmcjson.TxRawResult, error) {
	...
	txReply := &ohmcjson.TxRawResult{
		Hex:      mtxHex,
		Txid:     txHash,
		Hash:     mtx.WitnessHash().String(),
		Size:     int32(mtx.SerializeSize()),
		Vsize:    int32(mempool.GetTxVirtualSize(ohmcutil.NewTx(mtx))),
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ohmcjson.DecodeRawTransactionCmd)
	...
	if err != nil {
		return nil, &ohmcjson.RPCError{
			Code:    ohmcjson.ErrRPCDeserialization,
			Message: "TX decode failed: " + err.Error(),
		}
	}

	// Create and return the result.
	txReply := ohmcjson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ohmcjson.DecodeScriptCmd)
	...
	p2sh, err := ohmcutil.NewAddressScriptHash(script, s.cfg.ChainParams)
	if err != nil {
	...
	reply := ohmcjson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleGenerate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if len(cfg.miningAddrs) == 0 {
		return nil, &ohmcjson.RPCError{
			Code: ohmcjson.ErrRPCInternal.Code,
	...
	if !s.cfg.ChainParams.GenerateSupported {
		return nil, &ohmcjson.RPCError{
			Code: ohmcjson.ErrRPCDifficulty,
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ohmcjson.GetAddedNodeInfoCmd)
	...
	if c.Node != nil {
		...
		if !found {
			return nil, &ohmcjson.RPCError{
				Code:    ohmcjson.ErrRPCClientNodeNotAdded,
	...
	results := make([]*ohmcjson.GetAddedNodeInfoResult, 0, len(peers))
	for _, rpcPeer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		peer := rpcPeer.ToPeer()
		var result ohmcjson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = ohmcjson.Bool(peer.Connected())
		...
		addrs := make([]ohmcjson.GetAddedNodeInfoResultAddr, 0, len(ipList))
		for _, ip := range ipList {
			var addr ohmcjson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBestBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := &ohmcjson.GetBestBlockResult{
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blk, err := ohmcutil.NewBlockFromBytes(blkBytes)
	if err != nil {
	...
	blockReply := ohmcjson.GetBlockVerboseResult{
		Hash:          c.Hash,
	...
	if c.VerboseTx == nil || !*c.VerboseTx {
		...
		rawTxns := make([]ohmcjson.TxRawResult, len(txns))
		for i, tx := range txns {
...
func handleGetBlockChainInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	chainInfo := &ohmcjson.GetBlockChainInfoResult{
		Chain:         params.Name,
		...
		Pruned:        false,
		Bip9SoftForks: make(map[string]*ohmcjson.Bip9SoftForkDescription),
	}
	...
	chainInfo.SoftForks = []*ohmcjson.SoftForkDescription{
		{
			ID:      "bip34",
			Version: 2,
			Reject: struct {
	...
	for deployment, deploymentDetails := range params.Deployments {
		...
		chainInfo.Bip9SoftForks[forkName] = &ohmcjson.Bip9SoftForkDescription{
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ohmcjson.GetBlockHashCmd)
	hash, err := s.cfg.Chain.BlockHashByHeight(int32(c.Index))
	if err != nil {
		return nil, &ohmcjson.RPCError{
			Code:    ohmcjson.ErrRPCOutOfRange,
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ohmcjson.GetBlockHeaderCmd)
	...
	params := s.cfg.ChainParams
	blockHeaderReply := ohmcjson.GetBlockHeaderVerboseResult{
		Hash:          c.Hash,
		Confirmations: int64(1 + best.Height - blockHeight),
		Height:        blockHeight,
		Version:       blockHeader.Version,
...
// state as a ohmcjson.GetBlockTemplateResult that is ready to be encoded to JSON
// and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(useCoinbaseValue bool, submitOld *bool) (*ohmcjson.GetBlockTemplateResult, error) {
	...
	transactions := make([]ohmcjson.GetBlockTemplateResultTx, 0, numTx-1)
	...
	for i, tx := range msgBlock.Transactions {
		...
		bTx := ohmcutil.NewTx(tx)
		resultTx := ohmcjson.GetBlockTemplateResultTx{
	...
	reply := ohmcjson.GetBlockTemplateResult{
	...
	if useCoinbaseValue {
		reply.CoinbaseAux = gohmcoinbaseAux
...
func handleGetBlockTemplateRequest(s *rpcServer, request *ohmcjson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !(cfg.RegressionTest || cfg.SimNet) &&
		...
		return nil, &ohmcjson.RPCError{
			Code:    ohmcjson.ErrRPCClientNotConnected,
	...
	if currentHeight != 0 && !s.cfg.SyncMgr.IsCurrent() {
		return nil, &ohmcjson.RPCError{
			Code:    ohmcjson.ErrRPCClientInInitialDownload,
...
// chainErrToGBTErrString converts an error returned from ohmcchain to a string
...
func handleGetBlockTemplateProposal(s *rpcServer, request *ohmcjson.TemplateRequest) (interface{}, error) {
	hexData := request.Data
	if hexData == "" {
		return false, &ohmcjson.RPCError{
			Code: ohmcjson.ErrRPCType,
...
func handleGetBlockTemplate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ohmcjson.GetBlockTemplateCmd)
	request := c.Request
...
func handleGetCFilter(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	if s.cfg.CfIndex == nil {
		return nil, &ohmcjson.RPCError{
			Code:    ohmcjson.ErrRPCNoCFIndex,
			Message: "The CF index must be enabled for this command",
		}
	}

	c := cmd.(*ohmcjson.GetCFilterCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetCFilterHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*ohmcjson.GetCFilterHeaderCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &ohmcjson.InfoChainResult{
		...
		RelayFee:        cfg.minRelayTxFee.Toohmc(),
	}
...
func handleGetMempoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &ohmcjson.GetMempoolInfoResult{
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := ohmcjson.NewGetNetworkHashPSCmd(nil, nil)
	...
	best := s.cfg.Chain.BestSnapshot()
	result := ohmcjson.GetMiningInfoResult{
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &ohmcjson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*ohmcjson.GetNetworkHashPSCmd)
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*ohmcjson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.ToPeer().StatsSnapshot()
		info := &ohmcjson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ohmcjson.GetRawMempoolCmd)
	mp := s.cfg.TxMemPool
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ohmcjson.GetRawTransactionCmd)
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if includeMempool && s.cfg.TxMemPool.HaveTransaction(txHash) {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &ohmcjson.RPCError{
				Code: ohmcjson.ErrRPCInvalidTxVout,
	...
	txOutReply := &ohmcjson.GetTxOutResult{
		BestBlock:     bestBlockHash,
		Confirmations: int64(confirmations),
		Value:         ohmcutil.Amount(value).Toohmc(),
		ScriptPubKey: ohmcjson.ScriptPubKeyResult{
			Asm:       disbuf,
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]ohmcjson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockchain.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]ohmcjson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinList[0].Sequence = txIn.Sequence
		return vinList, nil
	}
	...
	vinList := make([]ohmcjson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &ohmcjson.PrevOut{
				Addresses: encodedAddrs,
				Value:     ohmcutil.Amount(originTxOut.Value).Toohmc(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr ohmcutil.Address, numToSkip, numRequested uint32) ([]*ohmcutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*ohmcjson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	params := s.cfg.ChainParams
	addr, err := ohmcutil.DecodeAddress(c.Address, params)
	if err != nil {
		return nil, &ohmcjson.RPCError{
			Code:    ohmcjson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
	...
	// The verbose flag is set, so generate the JSON object and return it.
	best := s.cfg.Chain.BestSnapshot()
	srtList := make([]ohmcjson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ohmcjson.SendRawTransactionCmd)
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ohmcjson.ValidateAddressCmd)

	result := ohmcjson.ValidateAddressChainResult{}
	addr, err := ohmcutil.DecodeAddress(c.Address, s.cfg.ChainParams)
	if err != nil {
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ohmcjson.VerifyMessageCmd)
	...
	if _, ok := addr.(*ohmcutil.AddressPubKeyHash); !ok {
		return nil, &ohmcjson.RPCError{
			Code:    ohmcjson.ErrRPCType,
	...
	pk, wasCompressed, err := ohmcec.RecoverCompact(ohmcec.S256(), sig,
		expectedMessageHash)
	if err != nil {
	...
	address, err := ohmcutil.NewAddressPubKey(serializedPK, params)
	if err != nil {
...
//
// NOTE: This is a ohmcsuite extension ported from github.com/decred/dcrd.
func handleVersion(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	result := map[string]ohmcjson.VersionResult{
		"ohmcdjsonrpcapi": {
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, ohmcjson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *ohmcjson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(ohmcjson.Error); ok &&
			jerr.ErrorCode == ohmcjson.ErrUnregisteredMethod {

			parsedCmd.err = ohmcjson.ErrRPCMethodNotFound
			return &parsedCmd
		}

		// Otherwise, some type of invalid parameters is the
		// cause, so produce the equivalent RPC error.
		parsedCmd.err = ohmcjson.NewRPCError(
			ohmcjson.ErrRPCInvalidParams.Code, err.Error())
		return &parsedCmd
	}
...
// the type *ohmcjson.RPCError to the appropriate type as needed.
func createMarshalledReply(id, result interface{}, replyErr error) ([]byte, error) {
	var jsonErr *ohmcjson.RPCError
	if replyErr != nil {
		if jErr, ok := replyErr.(*ohmcjson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return ohmcjson.MarshalResponse(id, result, jsonErr)
}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="ohmcd RPC"`)
...
type rpcserverSyncManager interface {
	...
	SubmitBlock(block *ohmcutil.Block, flags blockchain.BehaviorFlags) (bool, error)
