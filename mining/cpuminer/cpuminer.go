...
type Config struct {
	...
	ProcessBlock func(*ohmcutil.Block, blockchain.BehaviorFlags) (bool, error)
...
func (m *CPUMiner) submitBlock(block *ohmcutil.Block) bool {
	...
	log.Infof("Block submitted via CPU miner accepted (hash %s, "+
		"amount %v)", block.Hash(), ohmcutil.Amount(coinbaseTx.Value))
	return true
}
...
out:
	for {
		...
		if m.solveBlock(template.Block, curHeight+1, ticker, quit) {
			block := ohmcutil.NewBlock(template.Block)
			m.submitBlock(block)
		}
	}
