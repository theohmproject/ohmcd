...
import (
	...
	"github.com/ohmcsuite/winsvc/eventlog"
	"github.com/ohmcsuite/winsvc/mgr"
	"github.com/ohmcsuite/winsvc/svc"
)

const (
	// svcName is the name of ohmcd service.
	svcName = "ohmcdsvc"
...
// logServiceStartOfDay logs information about ohmcd when the main server has
...
// ohmcdService houses the main service handler which handles all service
// updates and launching ohmcdMain.
type ohmcdService struct{}
...
// long-running ohmcdMain (which is the real meat of ohmcd), handles service
// change requests, and notifies the service control manager of changes.
func (s *ohmcdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start ohmcdMain in a separate goroutine so the service can start
...
// installService attempts to install the ohmcd service.  Typically this should
...
// removeService attempts to uninstall the ohmcd service.  Typically this should
...
// startService attempts to start the ohmcd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &ohmcdService{})
	if err != nil {
