...
ohmcd provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API that is
fully compatible with the original bitcoind/bitcoin-qt.  There are a few key
differences between ohmcd and bitcoind as far as how RPCs are serviced:
* Unlike bitcoind that has the wallet and chain intermingled in the same process
  which leads to several issues, ohmcd intentionally splits the wallet and chain
  services into independent processes.  See the blog post
  [here](https://blog.conformal.com/ohmcd-not-your-moms-bitcoin-daemon/) for
  further details on why they were separated.  This means that if you are
  talking directly to ohmcd, only chain-related RPCs are available.  However both
  chain-related and wallet-related RPCs are available via
  [ohmcwallet](https://github.com/ohmcsuite/ohmcwallet).
* ohmcd is secure by default which means that the RPC connection is TLS-enabled
  by default
* ohmcd provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for ohmcd RPC and are used by applications
such as [ohmcwallet](https://github.com/ohmcsuite/ohmcwallet) for inter-process
communication with ohmcd.  The websocket connection endpoint for ohmcd is
...
during the development of ohmcwallet.
...
The ohmcd RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
ohmcd is `wss://your_ip_or_domain:8334/ws`.
...
to a ohmcd RPC server:

* **rpcuser** is the full-access username configured for the ohmcd RPC server
* **rpcpass** is the full-access password configured for the ohmcd RPC server
* **rpclimituser** is the limited username configured for the ohmcd RPC server
* **rpclimitpass** is the limited password configured for the ohmcd RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the ohmcd
  server is configured with.  It is automatically generated by ohmcd and placed
  in the ohmcd home directory (which is typically `%LOCALAPPDATA%\ohmcd` on
  Windows and `~/.ohmcd` on POSIX-like OSes)

**NOTE:** As mentioned above, ohmcd is secure by default which means the RPC
...
The ohmcd RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
ohmcd comes with a separate utility named `ohmcctl` which can be used to issue
these RPC commands via HTTP POST requests to ohmcd after configuring it with the
...
|25|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><font color="orange">ohmcd does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
|26|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Since ohmcd does not have the wallet integrated to provide payment addresses, ohmcd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|27|[stop](#stop)|N|Shutdown ohmcd.|
|28|[submitblock](#submitblock)|Y|Attempts to submit a new serialized, hex-encoded block to the network.|
|29|[validateaddress](#validateaddress)|Y|Verifies the given address is valid.  NOTE: Since ohmcd does not have a wallet integrated, ohmcd will only return whether the address is valid or not.|
...
|---|---|
|Method|getblock|
|Parameters|1. block hash (string, required) - the hash of the block<br />2. verbose (boolean, optional, default=true) - specifies the block is returned as a JSON object instead of hex-encoded string<br />3. verbosetx (boolean, optional, default=false) - specifies that each transaction is returned as a JSON object and only applies if the `verbose` flag is true.<font color="orange">**This parameter is a ohmcd extension**</font>|
...
|Notes|<font color="orange">Since ohmcd does not perform any mining, the priority related fields `startingpriority` and `currentpriority` that are available when the `verbose` flag is set are always 0.</font>|
...
|Notes|NOTE: Since ohmcd does not have the wallet integrated to provide payment addresses, ohmcd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|<font color="orange">ohmcd does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown ohmcd.|
|Returns|`"ohmcd stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database.<br />The actual checks performed by the `checklevel` parameter is implementation specific.  For ohmcd this is:<br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|<font color="orange">ohmcd currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for ohmcd.</font>|
...
The following is an overview of the RPC methods which are implemented by ohmcd, but not the original bitcoind client. Click the method name for further details such as parameter and return information.
...
|3|[getcurrentnet](#getcurrentnet)|Y|Get bitcoin network ohmcd is running on.|None|
...
|Description|Dynamically changes the debug logging level.<br />The levelspec can either a debug level or of the form `<subsystem>=<level>,<subsystem2>=<level2>,...`<br />The valid debug levels are `trace`, `debug`, `info`, `warn`, `error`, and `critical`.<br />The valid subsystems are `AMGR`, `ADXR`, `BCDB`, `BMGR`, `ohmcd`, `CHAN`, `DISC`, `PEER`, `RPCS`, `SCRP`, `SRVR`, and `TXMP`.<br />Additionally, the special keyword `show` can be used to get a list of the available subsystems.|
|Returns|string|
|Example Return|`Done.`|
|Example `show` Return|`Supported subsystems [AMGR ADXR BCDB BMGR ohmcd CHAN DISC PEER RPCS SCRP SRVR TXMP]`|
...
|Parameters|None|
|Description|Get bitcoin network ohmcd is running on.|
|Returns|numeric|
...
|Method|version|
|Parameters|None|
|Description|Returns the version of the JSON-RPC API built into this release of ohmcd.|
|Returns|`{ (json object)`<br />&nbsp;&nbsp;`"ohmcdjsonrpcapi": {`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"versionstring": "x.y.z",  (string) the version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"major": x,  (numeric) the major version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"minor": y,  (numeric) the minor version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"patch": z,  (numeric) the patch version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"prerelease": "",  (string) prerelease info for the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"buildmetadata": ""  (string) metadata about the server build`<br />&nbsp;&nbsp;`}`<br />`}`|
|Example Return|`{`<br />&nbsp;&nbsp;`"ohmcdjsonrpcapi": {`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"versionstring": "1.0.0",`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"major": 1,  `<br />&nbsp;&nbsp;&nbsp;&nbsp;`"minor": 0,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"patch": 0,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"prerelease": "",`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"buildmetadata": ""`<br />&nbsp;&nbsp;`}`<br />`}`|
[Return to Overview](#MethodOverview)<br />
...
|Description|*DEPRECATED, for similar functionality see [loadtxfilter](#loadtxfilter)*<br />Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this ohmcd instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSExtMethodOverview)<br />
...
ohmcd uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll ohmcd for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
[ohmcrpcclient](https://github.com/ohmcsuite/ohmcrpcclient) package.
...
[ohmcrpcclient](https://github.com/ohmcsuite/ohmcrpcclient) package to connect with
a ohmcd instance via Websockets, issues [getblockcount](#getblockcount) to
...
import (
	"github.com/ohmcsuite/ohmcrpcclient"
...
func main() {
	...
	// generated by ohmcd when it starts the RPC server and doesn't already
	// have one.
	...
	// exits.
	connCfg := &ohmcrpcclient.ConnConfig{
		Host:         "localhost:8334",
		Endpoint:     "ws",
		User:         "yourrpcuser",
		Pass:         "yourrpcpass",
	...
	client, err := ohmcrpcclient.New(connCfg, nil)
	if err != nil {
		log.Fatal(err)
	}
...
The following is an example Go application which uses the
[ohmcrpcclient](https://github.com/ohmcsuite/ohmcrpcclient) package to connect with
a ohmcd instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[ohmcrpcclient](https://github.com/ohmcsuite/ohmcrpcclient) package to connect with
a ohmcd instance via Websockets and registers for
...
func main() {
	...
	// notifications.
	ntfnHandlers := ohmcrpcclient.NotificationHandlers{
	...
	client, err := ohmcrpcclient.New(connCfg, &ntfnHandlers)
	if err != nil {
		log.Fatal(err)
	}
...
(can be installed with `npm install ws`) to connect with a ohmcd instance,
...
// generated by ohmcd when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/ohmcd/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The ohmcd generated certificate acts as
