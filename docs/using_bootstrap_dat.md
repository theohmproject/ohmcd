...
5. [How do I use bootstrap.dat with ohmcd?](#Importing)
...
**NOTE:** Using bootstrap.dat is entirely optional.  ohmcd will download the
...
### 5. How do I use bootstrap.dat with ohmcd?

ohmcd comes with a separate utility named `addblock` which can be used to import
...
1. Stop ohmcd if it is already running.  This is required since addblock needs to
   access the database used by ohmcd and it will be locked if ohmcd is using it.
...
**Windows:**
```bat
C:\> "%PROGRAMFILES%\ohmcd Suite\ohmcd\addblock" -i C:\Path\To\bootstrap.dat
```
