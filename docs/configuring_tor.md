...
ohmcd provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, ohmcd supports a hybrid,
...
Configuring ohmcd as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the ohmcd command
line or in the ohmcd configuration file.  Typically the Tor proxy address will be
...
By default, ohmcd assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since ohmcd will act as the server.
...
```text
HiddenServiceDir /var/tor/ohmcd
HiddenServicePort 8333 127.0.0.1:8333
```

Once Tor is configured to provide the hidden service and you have obtained your
generated .onion address, configuring ohmcd as a Tor hidden service requires
three flags:
...
```bash
$ ./ohmcd --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
ohmcd provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the ohmcd command line or in the ohmcd
...
```bash
$ ./ohmcd --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
ohmcd provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./ohmcd --proxy=127.0.0.1:9050 --torisolation
```
