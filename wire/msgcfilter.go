...
func (msg *MsgCFilter) ohmcDecode(r io.Reader, pver uint32, _ MessageEncoding) error {
	// Read filter type
...
// This is part of the Message interface implementation.
func (msg *MsgCFilter) ohmcEncode(w io.Writer, pver uint32, _ MessageEncoding) error {
	size := len(msg.Data)
	if size > MaxCFilterDataSize {
		str := fmt.Sprintf("cfilter size too large for message "+
			"[size %v, max %v]", size, MaxCFilterDataSize)
		return messageError("MsgCFilter.ohmcEncode", str)
	}
...
// from ohmcDecode in that ohmcDecode decodes from the bitcoin wire protocol as
