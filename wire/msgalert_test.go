...
func TestMsgAlert(t *testing.T) {
	...
	// Test ohmcEncode with Payload == nil
	var buf bytes.Buffer
	err := msg.ohmcEncode(&buf, pver, encoding)
	if err != nil {
		t.Error(err.Error())
	}
	...
	if !bytes.Equal(buf.Bytes(), expectedBuf) {
		t.Errorf("ohmcEncode got: %s want: %s",
...
func TestMsgAlertWire(t *testing.T) {
	...
	for i, test := range tests {
		...
		var buf bytes.Buffer
		err := test.in.ohmcEncode(&buf, test.pver, test.enc)
		if err != nil {
			t.Errorf("ohmcEncode #%d error %v", i, err)
			continue
		}
		if !bytes.Equal(buf.Bytes(), test.buf) {
			t.Errorf("ohmcEncode #%d\n got: %s want: %s", i,
		...
		err = msg.ohmcDecode(rbuf, test.pver, test.enc)
		if err != nil {
			t.Errorf("ohmcDecode #%d error %v", i, err)
			continue
		}
		if !reflect.DeepEqual(&msg, test.out) {
			t.Errorf("ohmcDecode #%d\n got: %s want: %s", i,
...
func TestMsgAlertWireErrors(t *testing.T) {
	...
	for i, test := range tests {
		...
		err := test.in.ohmcEncode(w, test.pver, test.enc)
		if reflect.TypeOf(err) != reflect.TypeOf(test.writeErr) {
			t.Errorf("ohmcEncode #%d wrong error got: %v, want: %v",
				i, err, test.writeErr)
			continue
		}
		...
		if _, ok := err.(*MessageError); !ok {
			if err != test.writeErr {
				t.Errorf("ohmcEncode #%d wrong error got: %v, "+
		...
		err = msg.ohmcDecode(r, test.pver, test.enc)
		if reflect.TypeOf(err) != reflect.TypeOf(test.readErr) {
			t.Errorf("ohmcDecode #%d wrong error got: %v, want: %v",
				i, err, test.readErr)
			continue
		}
		...
		if _, ok := err.(*MessageError); !ok {
			if err != test.readErr {
				t.Errorf("ohmcDecode #%d wrong error got: %v, "+
	...
	w := new(bytes.Buffer)
	err := baseMsgAlert.ohmcEncode(w, pver, encoding)
	if _, ok := err.(*MessageError); !ok {
		t.Errorf("MsgAlert.ohmcEncode wrong error got: %T, want: %T",
			err, MessageError{})
	}
	...
	buf := *new(bytes.Buffer)
	err = baseMsgAlert.ohmcEncode(&buf, pver, encoding)
	if _, ok := err.(*MessageError); !ok {
		t.Errorf("MsgAlert.ohmcEncode wrong error got: %T, want: %T",
			err, MessageError{})
	}
