...
func TestRejectLatest(t *testing.T) {
	...
	err := msg.ohmcEncode(&buf, pver, enc)
	if err != nil {
	...
	readMsg := MsgReject{}
	err = readMsg.ohmcDecode(&buf, pver, enc)
	if err != nil {
...
func TestRejectCrossProtocol(t *testing.T) {
	...
	var buf bytes.Buffer
	err := msg.ohmcEncode(&buf, ProtocolVersion, BaseEncoding)
	if err != nil {
		t.Errorf("encode of MsgReject failed %v err <%v>", msg, err)
	}
	...
	readMsg := MsgReject{}
	err = readMsg.ohmcDecode(&buf, RejectVersion-1, BaseEncoding)
	if err == nil {
		t.Errorf("encode of MsgReject succeeded when it shouldn't "+
			"have %v", msg)
	}
...
func TestRejectWire(t *testing.T) {
	...
	for i, test := range tests {
		...
		var buf bytes.Buffer
		err := test.msg.ohmcEncode(&buf, test.pver, test.enc)
		if err != nil {
