...
func (msg *MsgGetData) ohmcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if count > MaxInvPerMsg {
		...
		return messageError("MsgGetData.ohmcDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgGetData) ohmcEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	// Limit to max inventory vectors per message.
	count := len(msg.InvList)
	if count > MaxInvPerMsg {
		str := fmt.Sprintf("too many invvect in message [%v]", count)
		return messageError("MsgGetData.ohmcEncode", str)
	}
