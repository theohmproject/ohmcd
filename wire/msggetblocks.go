...
func (msg *MsgGetBlocks) ohmcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if count > MaxBlockLocatorsPerMsg {
		...
		return messageError("MsgGetBlocks.ohmcDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgGetBlocks) ohmcEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	count := len(msg.BlockLocatorHashes)
	if count > MaxBlockLocatorsPerMsg {
		str := fmt.Sprintf("too many block locator hashes for message "+
			"[count %v, max %v]", count, MaxBlockLocatorsPerMsg)
		return messageError("MsgGetBlocks.ohmcEncode", str)
	}
