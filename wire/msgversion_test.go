...
func TestVersionWireErrors(t *testing.T) {
	...
	// Ensure calling MsgVersion.ohmcDecode with a non *bytes.Buffer returns
	// error.
	fr := newFixedReader(0, []byte{})
	if err := baseVersion.ohmcDecode(fr, pver, enc); err == nil {
		t.Errorf("Did not received error when calling " +
			"MsgVersion.ohmcDecode with non *bytes.Buffer")
	}
	...
	for i, test := range tests {
		...
		err = msg.ohmcDecode(buf, test.pver, test.enc)
...
func TestVersionOptionalFields(t *testing.T) {
	...
	uaVersion.UserAgent = "/ohmcdtest:0.0.1/"
