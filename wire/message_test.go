...
func makeHeader(ohmcnet BitcoinNet, command string,
	...
	buf := make([]byte, 24)
	binary.LittleEndian.PutUint32(buf, uint32(ohmcnet))
...
func TestMessage(t *testing.T) {
	...
	tests := []struct {
		...
		ohmcnet BitcoinNet // Network to use for wire encoding
	...
	for i, test := range tests {
		...
		nw, err := WriteMessageN(&buf, test.in, test.pver, test.ohmcnet)
		if err != nil {
		...
		nr, msg, _, err := ReadMessageN(rbuf, test.pver, test.ohmcnet)
		if err != nil {
	...
	for i, test := range tests {
		// Encode to wire format.
		var buf bytes.Buffer
		err := WriteMessage(&buf, test.in, test.pver, test.ohmcnet)
		if err != nil {
			t.Errorf("WriteMessage #%d error %v", i, err)
			continue
		}
		...
		// Decode from wire format.
		rbuf := bytes.NewReader(buf.Bytes())
		msg, _, err := ReadMessage(rbuf, test.pver, test.ohmcnet)
		if err != nil {
			t.Errorf("ReadMessage #%d error %v, msg %v", i, err,
				spew.Sdump(msg))
			continue
...
func TestReadMessageWireErrors(t *testing.T) {
	...
	// otherwise accurate.
	badMessageBytes := makeHeader(ohmcnet, "addr", 1, 0xeaadc31c)
	...
	discardBytes := makeHeader(ohmcnet, "bogus", 15*1024, 0)

	tests := []struct {
		buf     []byte     // Wire encoding
		pver    uint32     // Protocol version for wire encoding
		ohmcnet  BitcoinNet // Bitcoin network for wire encoding
	...
	for i, test := range tests {
		...
		nr, _, _, err := ReadMessageN(r, test.pver, test.ohmcnet)
...
func TestWriteMessageWireErrors(t *testing.T) {
	...
	tests := []struct {
		...
		// Command too long.
		{badCommandMsg, pver, ohmcnet, 0, wireErr, 0},
		// Force error in payload encode.
		{encodeErrMsg, pver, ohmcnet, 0, wireErr, 0},
		// Force error due to exceeding max overall message payload size.
		{exceedOverallPayloadErrMsg, pver, ohmcnet, 0, wireErr, 0},
		// Force error due to exceeding max payload for message type.
		{exceedPayloadErrMsg, pver, ohmcnet, 0, wireErr, 0},
		// Force error in header write.
		{bogusMsg, pver, ohmcnet, 0, io.ErrShortWrite, 0},
		// Force error in payload write.
		{bogusMsg, pver, ohmcnet, 24, io.ErrShortWrite, 24},
	}
	...
	for i, test := range tests {
		...
		nw, err := WriteMessageN(w, test.msg, test.pver, test.ohmcnet)
