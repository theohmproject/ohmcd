...
type Message interface {
	ohmcDecode(io.Reader, uint32, MessageEncoding) error
	ohmcEncode(io.Writer, uint32, MessageEncoding) error
	Command() string
...
func WriteMessageN(w io.Writer, msg Message, pver uint32, ohmcnet BitcoinNet) (int, error) {
	return WriteMessageWithEncodingN(w, msg, pver, ohmcnet, BaseEncoding)
}
...
func WriteMessage(w io.Writer, msg Message, pver uint32, ohmcnet BitcoinNet) error {
	_, err := WriteMessageN(w, msg, pver, ohmcnet)
	return err
}
...
func WriteMessageWithEncodingN(w io.Writer, msg Message, pver uint32,
	ohmcnet BitcoinNet, encoding MessageEncoding) (int, error) {
	...
	var bw bytes.Buffer
	err := msg.ohmcEncode(&bw, pver, encoding)
	if err != nil {
...
func ReadMessageWithEncodingN(r io.Reader, pver uint32, ohmcnet BitcoinNet,
	...
	// MsgVersion ohmcDecode function requires it.
	pr := bytes.NewBuffer(payload)
	err = msg.ohmcDecode(pr, pver, enc)
	if err != nil {
		return totalBytes, nil, nil, err
	}
...
func ReadMessageN(r io.Reader, pver uint32, ohmcnet BitcoinNet) (int, Message, []byte, error) {
	return ReadMessageWithEncodingN(r, pver, ohmcnet, BaseEncoding)
}
...
func ReadMessage(r io.Reader, pver uint32, ohmcnet BitcoinNet) (Message, []byte, error) {
	_, msg, buf, err := ReadMessageN(r, pver, ohmcnet)
	return msg, buf, err
}
