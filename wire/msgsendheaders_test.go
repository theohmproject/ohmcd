...
func TestSendHeaders(t *testing.T) {
	...
	err = msg.ohmcEncode(&buf, oldPver, enc)
	if err == nil {
	...
	// exist yet.
	err = readmsg.ohmcDecode(&buf, oldPver, enc)
	if err == nil {
...
func TestSendHeadersCrossProtocol(t *testing.T) {
	...
	var buf bytes.Buffer
	err := msg.ohmcEncode(&buf, ProtocolVersion, enc)
	if err != nil {
		t.Errorf("encode of MsgSendHeaders failed %v err <%v>", msg,
			err)
	}
	...
	readmsg := NewMsgSendHeaders()
	err = readmsg.ohmcDecode(&buf, SendHeadersVersion, enc)
	if err != nil {
		t.Errorf("decode of MsgSendHeaders failed [%v] err <%v>", buf,
			err)
	}
