...
func (msg *MsgCFHeaders) ohmcDecode(r io.Reader, pver uint32, _ MessageEncoding) error {
	// Read filter type
	...
	if count > MaxCFHeadersPerMsg {
		str := fmt.Sprintf("too many committed filter headers for "+
			...
			MaxBlockHeadersPerMsg)
		return messageError("MsgCFHeaders.ohmcDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgCFHeaders) ohmcEncode(w io.Writer, pver uint32, _ MessageEncoding) error {
	// Write filter type
	...
	if count > MaxCFHeadersPerMsg {
		str := fmt.Sprintf("too many committed filter headers for "+
			"message [count %v, max %v]", count,
			MaxBlockHeadersPerMsg)
		return messageError("MsgCFHeaders.ohmcEncode", str)
	}
...
// differs from ohmcDecode in that ohmcDecode decodes from the bitcoin wire
