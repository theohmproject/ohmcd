...
func (msg *MsgHeaders) ohmcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if count > MaxBlockHeadersPerMsg {
		...
		return messageError("MsgHeaders.ohmcDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgHeaders) ohmcEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	// Limit to max block headers per message.
	count := len(msg.Headers)
	if count > MaxBlockHeadersPerMsg {
		str := fmt.Sprintf("too many block headers for message "+
			"[count %v, max %v]", count, MaxBlockHeadersPerMsg)
		return messageError("MsgHeaders.ohmcEncode", str)
	}
