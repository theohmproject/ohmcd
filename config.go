...
import (
	...
	"github.com/ohmcsuite/ohmcd/connmgr"
	"github.com/ohmcsuite/ohmcd/database"
	_ "github.com/ohmcsuite/ohmcd/database/ffldb"
...
const (
	defaultConfigFilename        = "ohmcd.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "ohmcd.log"
	defaultMaxPeers              = 125
	...
	sampleConfigFilename         = "sample-ohmcd.conf"
	defaultTxIndex               = false
	defaultAddrIndex             = false
)

var (
	defaultHomeDir     = ohmcutil.AppDataDir("ohmcd", false)
...
// config defines the configuration options for ohmcd.
//
...
type config struct {
	...
	MinRelayTxFee        float64       `long:"minrelaytxfee" description:"The minimum transaction fee in ohmc/kB to be considered a non-zero fee."`
...
//
// The above results in ohmcd functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	cfg := config{
		...
		MinRelayTxFee:        mempool.DefaultMinRelayTxFee.Toohmc(),
	...
	if cfg.DisableRPC {
		ohmcdLog.Infof("RPC service is disabled")
	}
	...
	cfg.minRelayTxFee, err = ohmcutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]ohmcutil.Address, 0, len(cfg.MiningAddrs))
	for _, strAddr := range cfg.MiningAddrs {
		addr, err := ohmcutil.DecodeAddress(strAddr, activeNetParams.Params)
		if err != nil {
	...
	if configFileError != nil {
		ohmcdLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the file sample-ohmcd.conf to the given destination path,
...
// ohmcdDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func ohmcdDial(addr net.Addr) (net.Conn, error) {
...
// ohmcdLookup resolves the IP of the given host using the correct DNS lookup
...
func ohmcdLookup(host string) ([]net.IP, error) {
