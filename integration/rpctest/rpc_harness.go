...
import (
	...
	"github.com/ohmcsuite/ohmcd/rpcclient"
...
// Harness fully encapsulates an active ohmcd process to provide a unified
// platform for creating rpc driven integration tests involving ohmcd. The
// active ohmcd node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active ohmcd process is fully
...
func New(activeNet *chaincfg.Params, handlers *rpcclient.NotificationHandlers,
	...
	if handlers.OnFilteredBlockConnected != nil {
		...
		handlers.OnFilteredBlockConnected = func(height int32, header *wire.BlockHeader, filteredTxns []*ohmcutil.Tx) {
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the ohmcd node itself. This spawns a new process which will be
	// managed
	...
	// wallet.
	filterAddrs := []ohmcutil.Address{h.wallet.coinbaseAddr}
	if err := h.Node.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure ohmcd properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created ohmcd
...
func (h *Harness) NewAddress() (ohmcutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() ohmcutil.Amount {
...
func (h *Harness) SendOutputs(targetOutputs []*wire.TxOut,
	feeRate ohmcutil.Amount) (*chainhash.Hash, error) {
...
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut,
	feeRate ohmcutil.Amount, change bool) (*wire.MsgTx, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) GenerateAndSubmitBlock(txns []*ohmcutil.Tx, blockVersion int32,
	blockTime time.Time) (*ohmcutil.Block, error) {
...
func (h *Harness) GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
	txns []*ohmcutil.Tx, blockVersion int32, blockTime time.Time,
	mineTo []wire.TxOut) (*ohmcutil.Block, error) {
	...
	prevBlock := ohmcutil.NewBlock(mBlock)
...
func baseDir() (string, error) {
	dirPath := filepath.Join(os.TempDir(), "ohmcd", "rpctest")
