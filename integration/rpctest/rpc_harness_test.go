...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt ohmcutil.Amount) *chainhash.Hash {
		...
		// Next, send amt ohmc to this address, spending from one of our mature
		// coinbase outputs.
	...
	// input.
	txid := genSpend(ohmcutil.Amount(5 * ohmcutil.SatoshiPerBitcoin))
	...
	txid = genSpend(ohmcutil.Amount(500 * ohmcutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(ohmcutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*ohmcutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, ohmcutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	// The internal wallet of this harness should now have 250 ohmc.
	expectedBalance := ohmcutil.Amount(250 * ohmcutil.SatoshiPerBitcoin)
	...
	// The original wallet should now have a balance of 0 ohmc as its entire
	// chain should have been decimated in favor of the main harness'
	// chain.
	expectedBalance = ohmcutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := ohmcutil.Amount(50 * ohmcutil.SatoshiPerBitcoin)
	...
	// The current wallet balance should now be at least 50 ohmc less
...
func TestHarness(t *testing.T) {
	// We should have (numMatureOutputs * 50 ohmc) of mature unspendable
	// outputs.
	expectedBalance := ohmcutil.Amount(numMatureOutputs * 50 * ohmcutil.SatoshiPerBitcoin)
