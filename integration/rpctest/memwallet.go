...
import (
	...
	"github.com/ohmcsuite/ohmcutil/hdkeychain"
)
...
type memWallet struct {
	coinbaseKey  *ohmcec.PrivateKey
	coinbaseAddr ohmcutil.Address
	...
	addrs map[uint32]ohmcutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]ohmcutil.Address)
...
// SetRPCClient saves the passed rpc connection to ohmcd as the wallet's
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*ohmcutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          ohmcutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (ohmcutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []ohmcutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt ohmcutil.Amount,
	feeRate ohmcutil.Amount, change bool) error {
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := ohmcutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	...
	var outputAmt ohmcutil.Amount
	for _, output := range outputs {
		outputAmt += ohmcutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() ohmcutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance ohmcutil.Amount
...
func keyToAddr(key *ohmcec.PrivateKey, net *chaincfg.Params) (ohmcutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := ohmcutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
