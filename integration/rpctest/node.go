...
import (
	...
	rpc "github.com/ohmcsuite/ohmcd/rpcclient"
	"github.com/ohmcsuite/ohmcutil"
)

// nodeConfig contains all the args, and data required to launch a ohmcd process
...
func newConfig(prefix, certFile, keyFile string, extra []string) (*nodeConfig, error) {
	ohmcdPath, err := ohmcdExecutablePath()
	if err != nil {
		ohmcdPath = "ohmcd"
	}
...
// arguments returns an array of arguments that be used to launch the ohmcd
// process.
...
// command returns the exec.Cmd which will be used to start the ohmcd process.
...
// to the ohmcd process that is launched via Start().
...
// as the base for the log and data directories for ohmcd.
...
// start creates a new ohmcd process, and writes its pid in a file reserved for
...
// stop interrupts the running ohmcd process process, and waits until it exits
...
// shutdown terminates the running ohmcd process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := ohmcutil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}
