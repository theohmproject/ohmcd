...
import (
	...
	"github.com/ohmcsuite/ohmcd/integration/rpctest"
...
func makeTestOutput(r *rpctest.Harness, t *testing.T,
	amt ohmcutil.Amount) (*ohmcec.PrivateKey, *wire.OutPoint, []byte, error) {
	...
	// spend.
	a, err := ohmcutil.NewAddressPubKey(key.PubKey().SerializeCompressed(), r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0113Activation(t *testing.T) {
	...
	ohmcdCfg := []string{"--rejectnonstd"}
	r, err := rpctest.New(&chaincfg.SimNetParams, nil, ohmcdCfg)
	if err != nil {
	...
	const outputValue = ohmcutil.SatoshiPerBitcoin
	...
	txns := []*ohmcutil.Tx{ohmcutil.NewTx(tx)}
...
func createCSVOutput(r *rpctest.Harness, t *testing.T,
	numSatoshis ohmcutil.Amount, timeLock int32,
	...
	p2shAddr, err := ohmcutil.NewAddressScriptHash(csvScript, r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0068AndBIP0112Activation(t *testing.T) {
	...
	const (
		outputAmt         = ohmcutil.SatoshiPerBitcoin
	...
	for txVersion := int32(0); txVersion < 3; txVersion++ {
		...
		txns := []*ohmcutil.Tx{ohmcutil.NewTx(spendingTx)}
		block, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
		if err != nil {
			t.Fatalf("unable to submit block: %v", err)
		}
	...
	for i, test := range tests {
		...
		if !test.accept {
			txns := []*ohmcutil.Tx{ohmcutil.NewTx(test.tx)}
			_, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
			if err == nil {
