...
func TestCalcMinRequiredTxRelayFee(t *testing.T) {
	tests := []struct {
		...
		relayFee ohmcutil.Amount // minimum relay transaction fee.
...
func TestCheckPkScriptStandard(t *testing.T) {
	var pubKeys [][]byte
	for i := 0; i < 4; i++ {
		pk, err := ohmcec.NewPrivateKey(ohmcec.S256())
		if err != nil {
...
func TestDust(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wire.TxOut{Value: ohmcutil.MaxSatoshi, PkScript: pkScript},
			ohmcutil.MaxSatoshi,
			false,
		},
		{
...
func TestCheckTransactionStandard(t *testing.T) {
	...
	addr, err := ohmcutil.NewAddressPubKeyHash(addrHash[:],
	...
	for _, test := range tests {
		...
		err := checkTransactionStandard(ohmcutil.NewTx(&test.tx),
