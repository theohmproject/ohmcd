...
type Config struct {
	...
	FetchUtxoView func(*ohmcutil.Tx) (*blockchain.UtxoViewpoint, error)
	...
	// utxo view.
	CalcSequenceLock func(*ohmcutil.Tx, *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error)
...
type Policy struct {
	...
	// MinRelayTxFee defines the minimum transaction fee in ohmc/kB to be
	// considered a non-zero fee.
	MinRelayTxFee ohmcutil.Amount
}
...
type TxPool struct {
	...
	orphansByPrev map[wire.OutPoint]map[chainhash.Hash]*ohmcutil.Tx
	outpoints     map[wire.OutPoint]*ohmcutil.Tx
...
func (mp *TxPool) removeOrphan(tx *ohmcutil.Tx, removeRedeemers bool) {
...
func (mp *TxPool) RemoveOrphan(tx *ohmcutil.Tx) {
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *ohmcutil.Tx, tag Tag) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		if _, exists := mp.orphansByPrev[txIn.PreviousOutPoint]; !exists {
			mp.orphansByPrev[txIn.PreviousOutPoint] =
				make(map[chainhash.Hash]*ohmcutil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *ohmcutil.Tx, tag Tag) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeOrphanDoubleSpends(tx *ohmcutil.Tx) {
	msgTx := tx.MsgTx()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *ohmcutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
	if removeRedeemers {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *ohmcutil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
	for _, txIn := range tx.MsgTx().TxIn {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint, tx *ohmcutil.Tx, height int32, fee int64) *TxDesc {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *ohmcutil.Tx) error {
	for _, txIn := range tx.MsgTx().TxIn {
...
func (mp *TxPool) CheckSpend(op wire.OutPoint) *ohmcutil.Tx {
	mp.mtx.RLock()
	txR := mp.outpoints[op]
	mp.mtx.RUnlock()
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *ohmcutil.Tx) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash) (*ohmcutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAcceptTransaction(tx *ohmcutil.Tx, isNew, rateLimit, rejectDupOrphans bool) ([]*chainhash.Hash, *TxDesc, error) {
	txHash := tx.Hash()
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(tx *ohmcutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, *TxDesc, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(acceptedTx *ohmcutil.Tx) []*TxDesc {
	...
	for processList.Len() > 0 {
		...
		processItem := firstElement.(*ohmcutil.Tx)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *ohmcutil.Tx, allowOrphan, rateLimit bool, tag Tag) ([]*TxDesc, error) {
...
// populated ohmcjson result.
//
// This function is safe for concurrent access.
func (mp *TxPool) RawMempoolVerbose() map[string]*ohmcjson.GetRawMempoolVerboseResult {
	mp.mtx.RLock()
	defer mp.mtx.RUnlock()

	result := make(map[string]*ohmcjson.GetRawMempoolVerboseResult,
		len(mp.pool))
	bestHeight := mp.cfg.BestHeight()
	...
	for _, desc := range mp.pool {
		...
		mpd := &ohmcjson.GetRawMempoolVerboseResult{
			Size:             int32(tx.MsgTx().SerializeSize()),
			Vsize:            int32(GetTxVirtualSize(tx)),
			Fee:              ohmcutil.Amount(desc.Fee).Toohmc(),
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphansByPrev:  make(map[wire.OutPoint]map[chainhash.Hash]*ohmcutil.Tx),
		nextExpireScan: time.Now().Add(orphanExpireScanInterval),
		outpoints:      make(map[wire.OutPoint]*ohmcutil.Tx),
	}
}
