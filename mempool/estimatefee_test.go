...
func (eft *estimateFeeTester) testTx(fee ohmcutil.Amount) *TxDesc {
	eft.version++
	return &TxDesc{
		TxDesc: mining.TxDesc{
			Tx: ohmcutil.NewTx(&wire.MsgTx{
				Version: eft.version,
			}),
			Height: eft.height,
			Fee:    int64(fee),
...
func expectedFeePerKilobyte(t *TxDesc) ohmcPerKilobyte {
	size := float64(t.TxDesc.Tx.MsgTx().SerializeSize())
	fee := float64(t.TxDesc.Fee)

	return SatoshiPerByte(fee / size).ToohmcPerKb()
}

func (eft *estimateFeeTester) newBlock(txs []*wire.MsgTx) {
	eft.height++

	block := ohmcutil.NewBlock(&wire.MsgBlock{
		Transactions: txs,
	})
...
func (eft *estimateFeeTester) estimates() [estimateFeeDepth]ohmcPerKilobyte {

	// Generate estimates
	var estimates [estimateFeeDepth]ohmcPerKilobyte
...
func (eft *estimateFeeTester) round(txHistory [][]*TxDesc,
	estimateHistory [][estimateFeeDepth]ohmcPerKilobyte,
	txPerRound, txPerBlock uint32) ([][]*TxDesc, [][estimateFeeDepth]ohmcPerKilobyte) {

	// generate new txs.
	var newTxs []*TxDesc
	for i := uint32(0); i < txPerRound; i++ {
		newTx := eft.testTx(ohmcutil.Amount(rand.Intn(1000000)))
...
func TestEstimateFeeRollback(t *testing.T) {
	...
	estimateHistory := [][estimateFeeDepth]ohmcPerKilobyte{eft.estimates()}
...
func (eft *estimateFeeTester) checkSaveAndRestore(
	previousEstimates [estimateFeeDepth]ohmcPerKilobyte) {
