...
const (
	...
	DefaultMinRelayTxFee = ohmcutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee ohmcutil.Amount) int64 {
	...
	if minFee < 0 || minFee > ohmcutil.MaxSatoshi {
		minFee = ohmcutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *ohmcutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee ohmcutil.Amount) bool {
...
func checkTransactionStandard(tx *ohmcutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee ohmcutil.Amount,
...
func GetTxVirtualSize(tx *ohmcutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
