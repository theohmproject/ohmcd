...
// ohmcPerKilobyte is number with units of bitcoins per kilobyte.
type ohmcPerKilobyte float64

// ToohmcPerKb returns a float value that represents the given
// SatoshiPerByte converted to satoshis per kb.
func (rate SatoshiPerByte) ToohmcPerKb() ohmcPerKilobyte {
	// If our rate is the error value, return that.
	if rate == SatoshiPerByte(-1.0) {
		return -1.0
	}

	return ohmcPerKilobyte(float64(rate) * bytePerKb * ohmcPerSatoshi)
}

// Fee returns the fee for a transaction of a given size for
// the given fee rate.
func (rate SatoshiPerByte) Fee(size uint32) ohmcutil.Amount {
	// If our rate is the error value, return that.
	if rate == SatoshiPerByte(-1) {
		return ohmcutil.Amount(-1)
	}

	return ohmcutil.Amount(float64(rate) * float64(size))
}

// NewSatoshiPerByte creates a SatoshiPerByte from an Amount and a
// size in bytes.
func NewSatoshiPerByte(fee ohmcutil.Amount, size uint32) SatoshiPerByte {
...
func (ef *FeeEstimator) ObserveTransaction(t *TxDesc) {
	...
	if _, ok := ef.observed[hash]; !ok {
		...
		ef.observed[hash] = &observedTransaction{
			hash:     hash,
			feeRate:  NewSatoshiPerByte(ohmcutil.Amount(t.Fee), size),
			observed: t.Height,
...
func (ef *FeeEstimator) RegisterBlock(block *ohmcutil.Block) error {
	ef.mtx.Lock()
	defer ef.mtx.Unlock()
	...
	transactions := make(map[*ohmcutil.Tx]struct{})
...
func (ef *FeeEstimator) EstimateFee(numBlocks uint32) (ohmcPerKilobyte, error) {
	ef.mtx.Lock()
	defer ef.mtx.Unlock()
	...
	return ef.cached[int(numBlocks)-1].ToohmcPerKb(), nil
}
