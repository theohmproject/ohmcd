...
func (s *fakeChain) FetchUtxoView(tx *ohmcutil.Tx) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) CalcSequenceLock(tx *ohmcutil.Tx,
...
// transactions.
func txOutToSpendableOut(tx *ohmcutil.Tx, outputNum uint32) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum},
		amount:   ohmcutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
type poolHarness struct {
	...
	signKey     *ohmcec.PrivateKey
	payAddr     ohmcutil.Address
	payScript   []byte
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int32, numOutputs uint32) (*ohmcutil.Tx, error) {
...
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput, numOutputs uint32) (*ohmcutil.Tx, error) {
	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput ohmcutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*ohmcutil.Tx, error) {
	txChain := make([]*ohmcutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, ohmcutil.NewTx(tx))
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	signKey, signPub := ohmcec.PrivKeyFromBytes(ohmcec.S256(), keyBytes)

	// Generate associated pay-to-script-hash address and resulting payment
	// script.
	pubKeyBytes := signPub.SerializeCompressed()
	payPubKeyAddr, err := ohmcutil.NewAddressPubKey(pubKeyBytes, chainParams)
	if err != nil {
		return nil, nil, err
	}
...
func testPoolMembership(tc *testContext, tx *ohmcutil.Tx, inOrphanPool, inTxPool bool) {
	txHash := tx.Hash()
...
func TestBasicOrphanRemoval(t *testing.T) {
	...
	nonChainedOrphanTx, err := harness.CreateSignedTx([]spendableOutput{{
		amount:   ohmcutil.Amount(5000000000),
