...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-ohmcd.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "ohmcd")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of ohmcd binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-ohmcd.conf")
