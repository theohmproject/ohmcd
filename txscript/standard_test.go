...
// newAddressPubKey returns a new ohmcutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) ohmcutil.Address {
	addr, err := ohmcutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new ohmcutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) ohmcutil.Address {
	addr, err := ohmcutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new ohmcutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) ohmcutil.Address {
	addr, err := ohmcutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the ohmcutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the ohmcutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := ohmcutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := ohmcutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := ohmcutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := ohmcutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := ohmcutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*ohmcutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*ohmcutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*ohmcutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*ohmcutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*ohmcutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
