...
// test data.
func genRandomSig() (*chainhash.Hash, *ohmcec.Signature, *ohmcec.PublicKey, error) {
	privKey, err := ohmcec.NewPrivateKey(ohmcec.S256())
	if err != nil {
...
func TestSigCacheAddExists(t *testing.T) {
	...
	sig1Copy, _ := ohmcec.ParseSignature(sig1.Serialize(), ohmcec.S256())
	key1Copy, _ := ohmcec.ParsePubKey(key1.SerializeCompressed(), ohmcec.S256())
...
func TestSigCacheAddEvictEntry(t *testing.T) {
	...
	for i := uint(0); i < sigCacheSize; i++ {
		...
		sigCopy, _ := ohmcec.ParseSignature(sig.Serialize(), ohmcec.S256())
		keyCopy, _ := ohmcec.ParsePubKey(key.SerializeCompressed(), ohmcec.S256())
	...
	sigNewCopy, _ := ohmcec.ParseSignature(sigNew.Serialize(), ohmcec.S256())
	keyNewCopy, _ := ohmcec.ParsePubKey(keyNew.SerializeCompressed(), ohmcec.S256())
