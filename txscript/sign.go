// Copyright (c) 2013-2015 The ohmcsuite developers
...
func RawTxInWitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int,
	...
	key *ohmcec.PrivateKey) ([]byte, error) {
...
// WitnessSignature creates an input witness stack for tx to spend ohmc sent
...
func WitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int, amt int64,
	subscript []byte, hashType SigHashType, privKey *ohmcec.PrivateKey,
	...
	pk := (*ohmcec.PublicKey)(&privKey.PublicKey)
	var pkData []byte
	if compress {
...
func RawTxInSignature(tx *wire.MsgTx, idx int, subScript []byte,
	hashType SigHashType, key *ohmcec.PrivateKey) ([]byte, error) {
...
// SignatureScript creates an input signature script for tx to spend ohmc sent
...
func SignatureScript(tx *wire.MsgTx, idx int, subscript []byte, hashType SigHashType, privKey *ohmcec.PrivateKey, compress bool) ([]byte, error) {
...
func p2pkSignatureScript(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType, privKey *ohmcec.PrivateKey) ([]byte, error) {
	sig, err := RawTxInSignature(tx, idx, subScript, hashType, privKey)
	if err != nil {
		return nil, err
	}
...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType,
	addresses []ohmcutil.Address, nRequired int, kdb KeyDB) ([]byte, bool) {
...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	ScriptClass, []ohmcutil.Address, int, error) {
...
func mergeScripts(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	pkScript []byte, class ScriptClass, addresses []ohmcutil.Address,
...
func mergeMultiSig(tx *wire.MsgTx, idx int, addresses []ohmcutil.Address,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		pSig, err := ohmcec.ParseDERSignature(tSig, ohmcec.S256())
		if err != nil {
			continue
		}
		...
		for _, addr := range addresses {
			...
			// bad input.
			pkaddr := addr.(*ohmcutil.AddressPubKey)
...
type KeyDB interface {
	GetKey(ohmcutil.Address) (*ohmcec.PrivateKey, bool, error)
}

// KeyClosure implements KeyDB with a closure.
type KeyClosure func(ohmcutil.Address) (*ohmcec.PrivateKey, bool, error)

// GetKey implements KeyDB by returning the result of calling the closure.
func (kc KeyClosure) GetKey(address ohmcutil.Address) (*ohmcec.PrivateKey,
	bool, error) {
	return kc(address)
}
...
type ScriptDB interface {
	GetScript(ohmcutil.Address) ([]byte, error)
}

// ScriptClosure implements ScriptDB with a closure.
type ScriptClosure func(ohmcutil.Address) ([]byte, error)

// GetScript implements ScriptDB by returning the result of calling the closure.
func (sc ScriptClosure) GetScript(address ohmcutil.Address) ([]byte, error) {
	return sc(address)
}
