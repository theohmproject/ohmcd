...
func main() {
	...
	method := args[0]
	usageFlags, err := ohmcjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := ohmcjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// ohmcjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(ohmcjson.Error); ok {
		...
		// The error is not a ohmcjson.Error and this really should not
	...
	marshalledJSON, err := ohmcjson.MarshalCmd(1, cmd)
	if err != nil {
