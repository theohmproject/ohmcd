...
Bitcoin RPC server that uses a ohmcd/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with ohmcd (https://github.com/ohmcsuite/ohmcd),
ohmcwallet (https://github.com/ohmcsuite/ohmcwallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, ohmcd and
ohmcwallet provide a websocket interface that is more efficient than the standard
...
ohmcd or ohmcwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by ohmcd and
ohmcwallet only uses a single connection that remains open and allows
...
All notifications provided by ohmcd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by ohmcd (and
ohmcwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that ohmcd intentionally separates the wallet
functionality into a separate process named ohmcwallet.  This means if you are
connected to the ohmcd RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, ohmcwallet provides pass through treatment
...
detected by type asserting the error in a *ohmcjson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

  amount, err := client.GetBalance("")
  if err != nil {
  	if jerr, ok := err.(*ohmcjson.RPCError); ok {
  		switch jerr.Code {
  		case ohmcjson.ErrRPCUnimplemented:
...
The following full-blown client examples are in the examples directory:
 ...
 - ohmcdwebsockets
   ...
   block count
 - ohmcwalletwebsockets
   Connects to a ohmcwallet RPC server using TLS-secured websockets, registers
